...
@section('pages')
  ...
  <main class="bg-[#14141F] flex flex-col py-20 px-[240px]">
    <div class="p-10 bg-[#00000099] rounded-xl mb-[60px] max-w-[1410px] relative overflow-hidden">
      <div class="bg-[#343444] w-full h-5 absolute right-0 bottom-0 z-10 rounded-b-xl"></div>
      <div class="flex">
        <div class="z-10 w-[294px] h-[294px] bg-[#7A798A] rounded-[20px]">
          <img class="rounded-[20px] w-[294px] h-[294px] object-fill" src="{{$user->getFirstMediaUrL('ProfilePic')}}" alt="profile picture">
        </div>
        ...
        <div class="flex gap-[30px] flex-wrap gap-y-10 max-w-[1410px]">
          @foreach($items as $item)
            <x-users-cards :item-id="$item->id" :item-img="$item->getFirstMediaUrL('items_images')" :item-title="$item->title" :user_id="$item->user->name" :item-price="$item->price" :profile-pic="$item->user->getFirstMediaUrL('ProfilePic')"></x-users-cards>
          @endforeach
        </div>
